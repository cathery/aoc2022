...
int main()
{
	...
	std::size_t FewestSteps = -1;

	// Iterate through every starting point, there aren't that many
	for (std::size_t Index = 0; Index != Tiles.size(); ++Index)
	{
		...
		Steps.assign(Tiles.size(), -1);

		TraverseMap(Index, 0);

		FewestSteps = std::min(FewestSteps, Steps[EndingIndex]);
	}

	std::cout << FewestSteps << std::endl;
}